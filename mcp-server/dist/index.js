...
server.tool('get_record', 'Fetch an Aha feature by its reference number (e.g. SYM-12345). Returns title, description, and metadata.', { reference_num: z.string().describe('Aha feature reference number, e.g. SYM-12345') }, async ({ reference_num }) => {
    try {
...
server.tool('search_documents', 'Search Aha requirements by keyword. Returns matching requirements for style and format context.', { query: z.string().describe('Search keywords, e.g. "claim status adjuster"') }, async ({ query }) => {
    try {
...
server.tool('create_requirement', 'Create a new child requirement under an Aha feature. Call once per requirement (one per product).', {
...
server.tool('update_requirement', 'Update an existing Aha child requirement by its reference number (e.g. SYM-12345-1).', {
    ...
    if (name === undefined && description === undefined) {
        return { content: [{ type: 'text', text: 'Error: at least one of name or description must be provided.' }], isError: true };
    }
    try {
